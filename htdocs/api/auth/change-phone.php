<?php
    ...
    try {
        ...
        if($checkUser == false || $checkUser->num_rows < 1){
            ...
            $sql = SQL()->query("update `users` set `phone` = '".$new_phone."', `updated_at` = '".date("Y-m-d H:i:s")."' where `username` = '".$username."'");
    
            if($sql) {
                echo '{"code": "00", "text": "Thay đổi số điện thoại thành công. Bạn sẽ bị đăng xuất sau 3s"}';
                session_start();
                session_destroy();
            } else {
